...
public class KeyTree implements Iterable<KeyTree.Node> {
     ...
     * @return the child that has the provided path or null if not found
     */
    public Node getPriority(final String path) {
     ...
     */
    public Node getOrAdd(final String path) {
    ...
    protected Node findParent(final Node parent, final int indent) {
        final Node last = parent.getLast();
    ...
    protected Node createNode(final Node parent, final int indent, final String key) {
    ...
    public class Node implements Iterable<Node> {
        ...
        protected Map<String, Node> indexByName; // allows repetitions
        protected Map<String, Node> priorityIndex; // nodes added programmatically (not parsed)
        protected Map<Integer, Node> indexByElementIndex; // parent list
         ...
         */
        protected Node get(final String path, boolean add, boolean priority) {
            Node node = null;
        ...
        protected Node add(final String key, final boolean priority) {
            int indent = this == KeyTree.this.root ? 0 : this.indent + KeyTree.this.options.indent();
            return this.add(indent, key, priority);
        }

        protected Node add(final int indent, final String key, final boolean priority) {
            final Node child = KeyTree.this.createNode(this, indent, key);
        ...
        protected void setPriority(final String key, final Node child) {
        ...
        private Node addIndexed(final int i) {
            Node child = null;
